...
public class SwiftHealthPlugin: NSObject, FlutterPlugin {
        ...
        -> [NSDictionary]?
    {
        ...
        case let categorySamples as [HKCategorySample]:
            var processedSamples = categorySamples

            // Filter sleep samples based on type
            if dataTypeKey == SLEEP_IN_BED {
                processedSamples = processedSamples.filter { $0.value == 0 }
            }
            if dataTypeKey == SLEEP_ASLEEP {
                processedSamples = processedSamples.filter { $0.value == 1 }
            }
            if dataTypeKey == SLEEP_AWAKE {
                processedSamples = processedSamples.filter { $0.value == 2 }
            }
            if dataTypeKey == SLEEP_LIGHT {
                processedSamples = processedSamples.filter { $0.value == 3 }
            }
            if dataTypeKey == SLEEP_DEEP {
                processedSamples = processedSamples.filter { $0.value == 4 }
            }
            if dataTypeKey == SLEEP_REM {
                processedSamples = processedSamples.filter { $0.value == 5 }
            }

            // Filter headache samples based on type
            if dataTypeKey == HEADACHE_UNSPECIFIED {
                processedSamples = processedSamples.filter { $0.value == 0 }
            }
            if dataTypeKey == HEADACHE_NOT_PRESENT {
                processedSamples = processedSamples.filter { $0.value == 1 }
            }
            if dataTypeKey == HEADACHE_MILD {
                processedSamples = processedSamples.filter { $0.value == 2 }
            }
            if dataTypeKey == HEADACHE_MODERATE {
                processedSamples = processedSamples.filter { $0.value == 3 }
            }
            if dataTypeKey == HEADACHE_SEVERE {
                processedSamples = processedSamples.filter { $0.value == 4 }
            }

            return processedSamples.map { sample -> NSDictionary in
        ...
        default:
            if #available(iOS 14.0, *), let ecgSamples = samples as? [HKElectrocardiogram] {
                return ecgSamples.map(fetchEcgMeasurements)
            }
            return nil
        }
    }
            ...
            case let .measurement(measurement):
                if let voltageQuantity = measurement.quantity(for: .appleWatchSimilarToLeadI) {
                    ...
                    voltageValues.append([
                        "voltage": voltage, "timeSinceSampleStart": timeSinceSampleStart,
                    ])
                }
            case .done:
                semaphore.signal()
    ...
    func initializeTypes() {
        ...
        unitDict[DECIBEL_A_WEIGHTED_SOUND_PRESSURE_LEVEL] =
        ...
        if #available(iOS 13.0, *) {
            ...
            dataTypesDict[BLOOD_OXYGEN] = HKSampleType.quantityType(
                forIdentifier: .oxygenSaturation)!
            dataTypesDict[RESPIRATORY_RATE] = HKSampleType.quantityType(
            ...
            dataTypesDict[BODY_MASS_INDEX] = HKSampleType.quantityType(
                forIdentifier: .bodyMassIndex)!
            dataTypesDict[BODY_TEMPERATURE] = HKSampleType.quantityType(
            ...
            dataTypesDict[DIETARY_FAT_MONOUNSATURATED] = HKSampleType.quantityType(
                forIdentifier: .dietaryFatMonounsaturated)!
            dataTypesDict[DIETARY_FAT_POLYUNSATURATED] = HKSampleType.quantityType(
                forIdentifier: .dietaryFatPolyunsaturated)!
            dataTypesDict[DIETARY_FAT_SATURATED] = HKSampleType.quantityType(
                forIdentifier: .dietaryFatSaturated)!
            dataTypesDict[DIETARY_CHOLESTEROL] = HKSampleType.quantityType(
                forIdentifier: .dietaryCholesterol)!
            dataTypesDict[DIETARY_VITAMIN_A] = HKSampleType.quantityType(
                forIdentifier: .dietaryVitaminA)!
            dataTypesDict[DIETARY_THIAMIN] = HKSampleType.quantityType(
                forIdentifier: .dietaryThiamin)!
            dataTypesDict[DIETARY_RIBOFLAVIN] = HKSampleType.quantityType(
                forIdentifier: .dietaryRiboflavin)!
            dataTypesDict[DIETARY_NIACIN] = HKSampleType.quantityType(
                forIdentifier: .dietaryNiacin)!
            dataTypesDict[DIETARY_PANTOTHENIC_ACID] = HKSampleType.quantityType(
                forIdentifier: .dietaryPantothenicAcid)!
            dataTypesDict[DIETARY_VITAMIN_B6] = HKSampleType.quantityType(
                forIdentifier: .dietaryVitaminB6)!
            dataTypesDict[DIETARY_BIOTIN] = HKSampleType.quantityType(
                forIdentifier: .dietaryBiotin)!
            dataTypesDict[DIETARY_VITAMIN_B12] = HKSampleType.quantityType(
                forIdentifier: .dietaryVitaminB12)!
            dataTypesDict[DIETARY_VITAMIN_C] = HKSampleType.quantityType(
                forIdentifier: .dietaryVitaminC)!
            dataTypesDict[DIETARY_VITAMIN_D] = HKSampleType.quantityType(
                forIdentifier: .dietaryVitaminD)!
            dataTypesDict[DIETARY_VITAMIN_E] = HKSampleType.quantityType(
                forIdentifier: .dietaryVitaminE)!
            dataTypesDict[DIETARY_VITAMIN_K] = HKSampleType.quantityType(
                forIdentifier: .dietaryVitaminK)!
            dataTypesDict[DIETARY_FOLATE] = HKSampleType.quantityType(
                forIdentifier: .dietaryFolate)!
            dataTypesDict[DIETARY_CALCIUM] = HKSampleType.quantityType(
                forIdentifier: .dietaryCalcium)!
            dataTypesDict[DIETARY_CHLORIDE] = HKSampleType.quantityType(
                forIdentifier: .dietaryChloride)!
            dataTypesDict[DIETARY_IRON] = HKSampleType.quantityType(forIdentifier: .dietaryIron)!
            dataTypesDict[DIETARY_MAGNESIUM] = HKSampleType.quantityType(
                forIdentifier: .dietaryMagnesium)!
            dataTypesDict[DIETARY_PHOSPHORUS] = HKSampleType.quantityType(
                forIdentifier: .dietaryPhosphorus)!
            dataTypesDict[DIETARY_POTASSIUM] = HKSampleType.quantityType(
                forIdentifier: .dietaryPotassium)!
            dataTypesDict[DIETARY_SODIUM] = HKSampleType.quantityType(
            ...
            dataTypesDict[DIETARY_CHROMIUM] = HKSampleType.quantityType(
                forIdentifier: .dietaryChromium)!
            dataTypesDict[DIETARY_COPPER] = HKSampleType.quantityType(
                forIdentifier: .dietaryCopper)!
            dataTypesDict[DIETARY_IODINE] = HKSampleType.quantityType(
                forIdentifier: .dietaryIodine)!
            dataTypesDict[DIETARY_MANGANESE] = HKSampleType.quantityType(
                forIdentifier: .dietaryManganese)!
            dataTypesDict[DIETARY_MOLYBDENUM] = HKSampleType.quantityType(
                forIdentifier: .dietaryMolybdenum)!
            dataTypesDict[DIETARY_SELENIUM] = HKSampleType.quantityType(
            ...
            dataTypesDict[INSULIN_DELIVERY] = HKSampleType.quantityType(
            ...
            dataTypesDict[DISTANCE_SWIMMING] = HKSampleType.quantityType(
                forIdentifier: .distanceSwimming)!
            dataTypesDict[DISTANCE_CYCLING] = HKSampleType.quantityType(
                forIdentifier: .distanceCycling)!
            dataTypesDict[FLIGHTS_CLIMBED] = HKSampleType.quantityType(
            ...
            dataTypesDict[MENSTRUATION_FLOW] = HKSampleType.categoryType(
                forIdentifier: .menstrualFlow)!

            dataTypesDict[EXERCISE_TIME] = HKSampleType.quantityType(
            ...
            characteristicsTypesDict[BIRTH_DATE] = HKObjectType.characteristicType(
                forIdentifier: .dateOfBirth)!
            characteristicsTypesDict[GENDER] = HKObjectType.characteristicType(
                forIdentifier: .biologicalSex)!
            characteristicsTypesDict[BLOOD_TYPE] = HKObjectType.characteristicType(
        ...
        if #available(iOS 11.0, *) {
            dataQuantityTypesDict[ACTIVE_ENERGY_BURNED] = HKQuantityType.quantityType(
                forIdentifier: .activeEnergyBurned)!
            dataQuantityTypesDict[BASAL_ENERGY_BURNED] = HKQuantityType.quantityType(
                forIdentifier: .basalEnergyBurned)!
            dataQuantityTypesDict[BLOOD_GLUCOSE] = HKQuantityType.quantityType(
                forIdentifier: .bloodGlucose)!
            dataQuantityTypesDict[BLOOD_OXYGEN] = HKQuantityType.quantityType(
                forIdentifier: .oxygenSaturation)!
            dataQuantityTypesDict[BLOOD_PRESSURE_DIASTOLIC] = HKQuantityType.quantityType(
                forIdentifier: .bloodPressureDiastolic)!
            dataQuantityTypesDict[BLOOD_PRESSURE_SYSTOLIC] = HKQuantityType.quantityType(
                forIdentifier: .bloodPressureSystolic)!
            dataQuantityTypesDict[BODY_FAT_PERCENTAGE] = HKQuantityType.quantityType(
                forIdentifier: .bodyFatPercentage)!
            dataQuantityTypesDict[LEAN_BODY_MASS] = HKSampleType.quantityType(
                forIdentifier: .leanBodyMass)!
            dataQuantityTypesDict[BODY_MASS_INDEX] = HKQuantityType.quantityType(
                forIdentifier: .bodyMassIndex)!
            dataQuantityTypesDict[BODY_TEMPERATURE] = HKQuantityType.quantityType(
                forIdentifier: .bodyTemperature)!

            // Nutrition
            dataQuantityTypesDict[DIETARY_CARBS_CONSUMED] = HKSampleType.quantityType(
                forIdentifier: .dietaryCarbohydrates)!
            dataQuantityTypesDict[DIETARY_CAFFEINE] = HKSampleType.quantityType(
                forIdentifier: .dietaryCaffeine)!
            dataQuantityTypesDict[DIETARY_ENERGY_CONSUMED] = HKSampleType.quantityType(
                forIdentifier: .dietaryEnergyConsumed)!
            dataQuantityTypesDict[DIETARY_FATS_CONSUMED] = HKSampleType.quantityType(
                forIdentifier: .dietaryFatTotal)!
            dataQuantityTypesDict[DIETARY_PROTEIN_CONSUMED] = HKSampleType.quantityType(
                forIdentifier: .dietaryProtein)!
            dataQuantityTypesDict[DIETARY_FIBER] = HKSampleType.quantityType(
                forIdentifier: .dietaryFiber)!
            dataQuantityTypesDict[DIETARY_SUGAR] = HKSampleType.quantityType(
                forIdentifier: .dietarySugar)!
            dataQuantityTypesDict[DIETARY_FAT_MONOUNSATURATED] = HKSampleType.quantityType(
                forIdentifier: .dietaryFatMonounsaturated)!
            dataQuantityTypesDict[DIETARY_FAT_POLYUNSATURATED] = HKSampleType.quantityType(
                forIdentifier: .dietaryFatPolyunsaturated)!
            dataQuantityTypesDict[DIETARY_FAT_SATURATED] = HKSampleType.quantityType(
                forIdentifier: .dietaryFatSaturated)!
            dataQuantityTypesDict[DIETARY_CHOLESTEROL] = HKSampleType.quantityType(
                forIdentifier: .dietaryCholesterol)!
            dataQuantityTypesDict[DIETARY_VITAMIN_A] = HKSampleType.quantityType(
                forIdentifier: .dietaryVitaminA)!
            dataQuantityTypesDict[DIETARY_THIAMIN] = HKSampleType.quantityType(
                forIdentifier: .dietaryThiamin)!
            dataQuantityTypesDict[DIETARY_RIBOFLAVIN] = HKSampleType.quantityType(
                forIdentifier: .dietaryRiboflavin)!
            dataQuantityTypesDict[DIETARY_NIACIN] = HKSampleType.quantityType(
                forIdentifier: .dietaryNiacin)!
            dataQuantityTypesDict[DIETARY_PANTOTHENIC_ACID] = HKSampleType.quantityType(
                forIdentifier: .dietaryPantothenicAcid)!
            dataQuantityTypesDict[DIETARY_VITAMIN_B6] = HKSampleType.quantityType(
                forIdentifier: .dietaryVitaminB6)!
            dataQuantityTypesDict[DIETARY_BIOTIN] = HKSampleType.quantityType(
                forIdentifier: .dietaryBiotin)!
            dataQuantityTypesDict[DIETARY_VITAMIN_B12] = HKSampleType.quantityType(
                forIdentifier: .dietaryVitaminB12)!
            dataQuantityTypesDict[DIETARY_VITAMIN_C] = HKSampleType.quantityType(
                forIdentifier: .dietaryVitaminC)!
            dataQuantityTypesDict[DIETARY_VITAMIN_D] = HKSampleType.quantityType(
                forIdentifier: .dietaryVitaminD)!
            dataQuantityTypesDict[DIETARY_VITAMIN_E] = HKSampleType.quantityType(
                forIdentifier: .dietaryVitaminE)!
            dataQuantityTypesDict[DIETARY_VITAMIN_K] = HKSampleType.quantityType(
                forIdentifier: .dietaryVitaminK)!
            dataQuantityTypesDict[DIETARY_FOLATE] = HKSampleType.quantityType(
                forIdentifier: .dietaryFolate)!
            dataQuantityTypesDict[DIETARY_CALCIUM] = HKSampleType.quantityType(
                forIdentifier: .dietaryCalcium)!
            dataQuantityTypesDict[DIETARY_CHLORIDE] = HKSampleType.quantityType(
                forIdentifier: .dietaryChloride)!
            dataQuantityTypesDict[DIETARY_IRON] = HKSampleType.quantityType(
                forIdentifier: .dietaryIron)!
            dataQuantityTypesDict[DIETARY_MAGNESIUM] = HKSampleType.quantityType(
                forIdentifier: .dietaryMagnesium)!
            dataQuantityTypesDict[DIETARY_PHOSPHORUS] = HKSampleType.quantityType(
                forIdentifier: .dietaryPhosphorus)!
            dataQuantityTypesDict[DIETARY_POTASSIUM] = HKSampleType.quantityType(
                forIdentifier: .dietaryPotassium)!
            dataQuantityTypesDict[DIETARY_SODIUM] = HKSampleType.quantityType(
                forIdentifier: .dietarySodium)!
            dataQuantityTypesDict[DIETARY_ZINC] = HKSampleType.quantityType(
                forIdentifier: .dietaryZinc)!
            dataQuantityTypesDict[DIETARY_WATER] = HKSampleType.quantityType(
                forIdentifier: .dietaryWater)!
            dataQuantityTypesDict[DIETARY_CHROMIUM] = HKSampleType.quantityType(
                forIdentifier: .dietaryChromium)!
            dataQuantityTypesDict[DIETARY_COPPER] = HKSampleType.quantityType(
                forIdentifier: .dietaryCopper)!
            dataQuantityTypesDict[DIETARY_IODINE] = HKSampleType.quantityType(
                forIdentifier: .dietaryIodine)!
            dataQuantityTypesDict[DIETARY_MANGANESE] = HKSampleType.quantityType(
                forIdentifier: .dietaryManganese)!
            dataQuantityTypesDict[DIETARY_MOLYBDENUM] = HKSampleType.quantityType(
                forIdentifier: .dietaryMolybdenum)!
            dataQuantityTypesDict[DIETARY_SELENIUM] = HKSampleType.quantityType(
                forIdentifier: .dietarySelenium)!

            dataQuantityTypesDict[ELECTRODERMAL_ACTIVITY] = HKQuantityType.quantityType(
                forIdentifier: .electrodermalActivity)!
            dataQuantityTypesDict[FORCED_EXPIRATORY_VOLUME] = HKQuantityType.quantityType(
                forIdentifier: .forcedExpiratoryVolume1)!
            dataQuantityTypesDict[HEART_RATE] = HKQuantityType.quantityType(
                forIdentifier: .heartRate)!
            dataQuantityTypesDict[HEART_RATE_VARIABILITY_SDNN] = HKQuantityType.quantityType(
                forIdentifier: .heartRateVariabilitySDNN)!
            dataQuantityTypesDict[HEIGHT] = HKQuantityType.quantityType(forIdentifier: .height)!
            dataQuantityTypesDict[RESTING_HEART_RATE] = HKQuantityType.quantityType(
                forIdentifier: .restingHeartRate)!
            dataQuantityTypesDict[STEPS] = HKQuantityType.quantityType(forIdentifier: .stepCount)!
            dataQuantityTypesDict[WAIST_CIRCUMFERENCE] = HKQuantityType.quantityType(
                forIdentifier: .waistCircumference)!
            dataQuantityTypesDict[WALKING_HEART_RATE] = HKQuantityType.quantityType(
                forIdentifier: .walkingHeartRateAverage)!
            dataQuantityTypesDict[WEIGHT] = HKQuantityType.quantityType(forIdentifier: .bodyMass)!
            dataQuantityTypesDict[DISTANCE_WALKING_RUNNING] = HKQuantityType.quantityType(
                forIdentifier: .distanceWalkingRunning)!
            dataQuantityTypesDict[DISTANCE_SWIMMING] = HKQuantityType.quantityType(
                forIdentifier: .distanceSwimming)!
            dataQuantityTypesDict[DISTANCE_CYCLING] = HKQuantityType.quantityType(
                forIdentifier: .distanceCycling)!
            dataQuantityTypesDict[FLIGHTS_CLIMBED] = HKQuantityType.quantityType(
                forIdentifier: .flightsClimbed)!
        ...
        if #available(iOS 13.6, *) {
            dataTypesDict[HEADACHE_UNSPECIFIED] = HKSampleType.categoryType(
                forIdentifier: .headache)!
            dataTypesDict[HEADACHE_NOT_PRESENT] = HKSampleType.categoryType(
                forIdentifier: .headache)!
        ...
        if #available(iOS 16.0, *) {
            dataTypesDict[ATRIAL_FIBRILLATION_BURDEN] = HKQuantityType.quantityType(
                forIdentifier: .atrialFibrillationBurden)!

            dataTypesDict[WATER_TEMPERATURE] = HKQuantityType.quantityType(
                forIdentifier: .waterTemperature)!
            dataTypesDict[UNDERWATER_DEPTH] = HKQuantityType.quantityType(
                forIdentifier: .underwaterDepth)!
        }
