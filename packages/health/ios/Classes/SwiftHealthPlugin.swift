...
public class SwiftHealthPlugin: NSObject, FlutterPlugin {
    ...
    for (index, key) in types.enumerated() {
      if key == NUTRITION {
        ...
        if let characteristicsType = characteristicsTypesDict[key] {
          ...
          case 1:
            throw PluginError(
              message: "Can not ask for reading permissions to the type of \(characteristicsType)")
          default:
            break
          }
        }
  ...
  func writeMeal(call: FlutterMethodCall, result: @escaping FlutterResult) throws {
    ...
    var metadata =
      ["HKFoodMeal": mealTypeString, HKMetadataKeyWasUserEntered: NSNumber(value: isManualEntry)]
      as [String: Any]
    if name != nil {
    ...
    for (key, identifier) in NUTRITION_KEYS {
      ...
      let unit =
        key == "calories"
        ? HKUnit.kilocalorie() : key == "water" ? HKUnit.literUnit(with: .milli) : HKUnit.gram()
      let nutritionSample = HKQuantitySample(
        type: HKSampleType.quantityType(forIdentifier: identifier)!,
        quantity: HKQuantity(unit: unit, doubleValue: unwrappedValue), start: dateFrom, end: dateTo,
        metadata: metadata)
  ...
  private func processData(samples: [HKSample]?, dataTypeKey: String, unit: HKUnit?) -> [[String: Any]]? {
  ...
  func getData(call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    let query = HKSampleQuery(
      ...
      let processedData = processData(samples: samplesOrNil, dataTypeKey: dataTypeKey, unit: unit)
      DispatchQueue.main.async {
